...
You need to assemble paired-end reads from the `short_reads` folder and save the output to a folder named `spades_output`. Limit resource usage to 5 GB of memory and 4 threads. By default, `metaSPAdes` performs an error correction on the input reads before assembling. 
...
All output files (including intermediate ones) can be found in the `spades_output` folder. 
