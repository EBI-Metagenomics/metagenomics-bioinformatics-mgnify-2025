...
`scaffolds.fasta` are the ones you are usually interested into (also for 
downstream analyses like binning and MAG generation). We will focus on 
`contigs.fasta` for this session, which is the same you are going to use
in the coming practicals. Contigs are ordered from the longest assembled
to the shortest. You can already try to infer the strongest taxonomic 
