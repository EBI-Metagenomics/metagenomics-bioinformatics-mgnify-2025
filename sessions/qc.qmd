...
This practical (and and the others) will generate quite a few output files from the different commands. It's therefore recommended you keep your results well organised into different subdirectories, starting with the creation of a `qc_results` directory that will contain everything else.
...
```bash
docker run --rm -it  -e DISPLAY=$DISPLAY  -v $DATADIR:/opt/data -v $RESDIR:/results -v /tmp/.X11-unix:/tmp/.X11-unix:rw -e DISPLAY=unix$DISPLAY microbiomeinformatics/biata-qc-assembly:v2021
```
...
```bash
mkdir -p /results/fastqc_results/oral
fastqc /opt/data/oral_human_example_1_splitaa.fastq.gz -o /results/fastqc_results/oral
fastqc /opt/data/oral_human_example_2_splitaa.fastq.gz -o /results/fastqc_results/oral
chown 1001 /results/fastqc_results/oral/*.html
```
:::
...
# Run
```bash
mkdir /results/multiqc_results/oral
multiqc /results/fastqc_results/oral -o /results/multiqc_results/oral
chown 1001 /results/multiqc_results/oral/*.html
```
:::
...
fastp --in1 /opt/data/oral_human_example_1_splitaa.fastq.gz \
      ...
      --out1 /results/cleaned/oral/oral_human_example_1_splitaa.trimmed.fastq.gz \
      --out2 /results/cleaned/oral/oral_human_example_2_splitaa.trimmed.fastq.gz \
      -l 50 --cut_right --cut_right_window_size 4 --cut_right_mean_quality 20 -t 1 \
      --detect_adapter_for_pe \
      --json /results/cleaned/oral/oral.fastp.json --html /results/cleaned/oral/oral.fastp.html
```
:::
...
# Run
```bash
bowtie2 -1 /results/cleaned/oral/oral_human_example_1_splitaa.trimmed.fastq.gz \
        -2 /results/cleaned/oral/oral_human_example_2_splitaa.trimmed.fastq.gz \
        -x /opt/data/decontamination/chr10_phix.index \
        --un-conc-gz  /results/cleaned/oral/oral_human_example.fastq.gz \
...
# Run
```bash
mv /results/cleaned/oral/oral_human_example.fastq.1.gz /results/cleaned/oral/oral_human_example_1_splitaa_trimmed_decontam.fastq.gz
mv /results/cleaned/oral/oral_human_example.fastq.2.gz /results/cleaned/oral/oral_human_example_2_splitaa_trimmed_decontam.fastq.gz
```
...
::: {.callout-step .callout-tip}
# Run FastQC
Using what you have learned previously, generate a FastQC report for each of the `*trimmed_decontam.fastq.gz` files. Output the new fastqc report files in the same `/results/fastqc_results/oral` directory as last time.
:::

::: {.callout-step .callout-tip}
# Run multiQC
Also generate a multiQC report, with `/results/fastqc_results/oral` as input. The reason we generated the new FastQC reports in the same directory is so that you can compare how the reads have changed after the quality filtering and decontamination steps in the same final multiqc report.

```bash
mkdir -p /results/final_multiqc_results/oral
...
blastn -query /opt/data/freshwater_sediment_contigs.fa -db /opt/data/decontamination/phix_blastDB -task megablast -word_size 28 -best_hit_overhang 0.1 -best_hit_score_edge 0.1 -dust yes -evalue 0.0001 -min_raw_gapped_score 100 -penalty -5 -soft_masking true -window_size 100 -outfmt 6 -out /results/blast_results/freshwater_blast_out.txt
```
:::
...
```bash
cat /opt/data/blast_outfmt6.txt /results/blast_results/freshwater_blast_out.txt > /results/blast_results/freshwater_blast_out_headers.txt
cat /results/blast_results/freshwater_blast_out_headers.txt
```
:::
