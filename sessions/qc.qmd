...
```bash
tail -n +2 mystery_reads_A.mseq | cut -f14 | sort | uniq -c | awk '{$1=$1};1' | sed 's/ /\t/g'> mystery_reads_A_counts.tsv
tail -n +2 mystery_reads_B.mseq | cut -f14 | sort | uniq -c | awk '{$1=$1};1' | sed 's/ /\t/g'> mystery_reads_B_counts.tsv
```
For example:
```bash
