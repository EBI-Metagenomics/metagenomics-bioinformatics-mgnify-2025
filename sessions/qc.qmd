...
have all the data you need in `/home/training/QC_session/quality`. If not, you should download and decompress the data as follows:
...
```
you should now have in your `/home/training/QC_session` directory, the following subdirectories (`quality` and `qc_results`). Make sure both are there before moving onto the next steps.
...
Navigate to Files → Home → QC_session → qc_results → multiqc_results → oral in your VM
