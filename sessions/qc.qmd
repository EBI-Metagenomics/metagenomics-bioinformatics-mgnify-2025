...
For this practical we will need to a sample dataset. Downoload and extract the files from the tarball.
...
# you should now have the quality subdirectory in your /home/training folder. All the data you will need to run the practicals will be in this subdirectory
...
```
you should now have in your `/home/training` directory, the following subdirectories (`quality` and `qc_results`). Make sure both are there before moving onto the next steps.
...
Navigate to home → quality → fastqc_results in your VM
...
export RESDIR={path to output directory}
```
