...
be in `/home/training/QC_session/qc_data`. Check if it is there. If not, go to the `Downloading data` section
...
For this practical we will need to a sample dataset. The sample dataset should already 
be in `/home/training/QC_session/qc_data`. If not, you should download and decompress the data as follows:

```bash
mkdir -p /home/training/QC_session
cd /home/training/QC_session

wget http://ftp.ebi.ac.uk/pub/databases/metagenomics/mgnify_courses/ebi_2020/qc_data.tar.gz
...
```
You should now have in your `/home/training/QC_session` directory, the following subdirectories (`qc_data` and `qc_results`). **Make sure both are there before moving onto the next steps. You might run into permission issues later if you have not created them properly.**

:::

For this tutorial, you'll need to move into the working directory (/home/training/qc_data) and start a Docker container. Set the variables `DATADIR` and `RESDIR` as instructed.

```bash
cd /home/training/QC_session/qc_data
chmod -R 777 /home/training/QC_session/qc_data
export DATADIR=/home/training/QC_session/qc_data
...
Going to `/opt/results/mystery_reads`, you will find two mystery FASTQ files. You have this information about them:
...
Change into the directory containing hte mystery reads, and run `ls -lh` to see some information about the files.
```bash
cd /opt/results/mystery_reads
ls -lh
# total 7.7M
