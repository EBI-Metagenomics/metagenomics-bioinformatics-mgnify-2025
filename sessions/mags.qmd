...
statistics by mapping reads to the contigs. To do this, we can use [bwa](http://bio-bwa.sourceforge.net/) and then the samtools software
[htslib.org](http://www.htslib.org/) to reformat the
...
* the results should be saved into the `contigs.fasta.metabat-bins2000` folder

* we want the bin file names to start with the prefix `bin`

* the minimum contig length should be 2000

Put your command together but please check the answer below before running it to make sure everything is correct.
...
@[html]{<style>
  ...
  .show-content:checked + .collapse-content {
    display: block;
  }
</style>}

[Toggle Code](#) <input type="checkbox" class="show-content" id="toggle-code">
...
:::{.callout-note}
We set the minimum contig size to 2000 using the `--minContig` parameter.
:::
