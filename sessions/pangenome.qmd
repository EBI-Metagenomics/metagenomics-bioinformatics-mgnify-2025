...
To prevent CELEBRIMBOR from re-running Bakta and CheckM, we must update the creation times of the input and output files using `touch`. For your own analysis, this is not recommended as you will run the full CELEBRIMBOR pipeline.
...
Note: we need the `--ignore-incomplete` flag here as we are not running Bakta or CheckM. For your own analysis, this option is not recommended.
...
```bash
cd ~/course_dir/work_dir/Day_5/pangenome/CELEBRIMBOR_results
```
