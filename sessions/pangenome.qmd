...
::: {.callout-important}
## Challenge
Try this with a maximum core (pi) distance of 0.05 and a maximum accessory (a) distance of 1. Try also with different values and compare the results. The command will be:
...
There actually isn't one 'perfect' cutoff, which is supported by population genetic
simulations (see this [paper](https://genome.cshlp.org/content/early/2023/05/30/gr.277395.122)). So we are acutally free to choose a cutoff that defines subspecies which
...
directory to see the clusters and the tree. 
...
It is faster to ['assign'](https://poppunk-docs.bacpop.org/query_assignment.html) new samples to an existing definition of
...
<https://www.bacpop.org/poppunk-databases>. If you have a fit for a new species
...
future users can make use of novel cluster assignments. Simply add
`--update-db` to the assignment command above. This is beyond the scope of this
practical but is documented [here](https://poppunk-docs.bacpop.org/query_assignment.html#updating-the-database) and [here](https://poppunk-docs.bacpop.org/model_distribution.html).
