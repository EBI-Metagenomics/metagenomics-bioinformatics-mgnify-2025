...
Use the sidebar to see the instructions for each part, or the arrows at
...
First, unpack the genomes we will be using in this step by
running the following commands:
```bash
tar xvf MAGs_to_build_model.gz
mkdir isolate_genomes && mv *.fasta isolate_genomes
```

This will unzip 112 _B. uniformis_ isolate genomes (i.e. not MAG data).
We are going to use these to define subspecies within the population
using [PopPUNK](https://genome.cshlp.org/content/29/2/304).
...
PopPUNK is already installed on the VM. Confirm this by activating the conda environment `conda activate poppunk` and running `poppunk
--version`.
...
First, we need to create a list of the input files. This needs to have
...
```bash
paste <(ls isolate_genomes | cut -d "." -f 1 ) <(ls isolate_genomes/MGYG*) > rfile.txt
```
...
From the output we can see that everything is in one cluster (In `Network Summary` there is one component) and so we
...
will allow you to visualise the strain cluster network, the components of which are the strains detected by PopPUNK.
...
MAGs from MGnify with >95% completeness and \<5% contamination.

PopPUNK distances are relatively robust to missing sequence content seen
in MAGs, but less able to deal with contamination.

Again, unzip and create the list of input files for poppunk:

```bash
tar xvf MAGs_to_assign.bz2
mv fasta mag_fastas
paste <(ls mag_fastas | cut -d "." -f 1 ) <(ls mag_fastas/*.fasta) > qfile.txt
```
...
CELEBRIMBOR is already install on the VM. Confirm this by activating the conda environment `conda activate celebrimbor`.

Unzip the genomes :

```bash
tar xvf MAGs_for_CELEBRIMBOR.tar.gz
```

This archive contains two directories. `CELEBRIMBOR_MAGs` contains the fasta files we will be analysing. 
`results` contains the [Bakta](https://github.com/oschwengers/bakta) output which was generated previously from these genomes. As this is
the slowest part of the analysis, we have provided them to allow you to generate results faster.
...
from re-running Bakta, we must update the creation times of the input and output files using `touch`.

```bash
(cd MAGs_for_CELEBRIMBOR/CELEBRIMBOR_MAGs && touch *.fasta)
(cd MAGs_for_CELEBRIMBOR/results && touch annotated/*/* && touch annotated/*)
```
...
```bash
#output directory
output_dir: /path/to/MAGs_for_CELEBRIMBOR/results

# dir with genome fasta files, must have '.fasta' extension, to convert use 'rename .fa .fasta  *.fa' (e.g. if extensions are .fa)
genome_fasta: /path/to/MAGs_for_CELEBRIMBOR/CELEBRIMBOR_MAGs

# path to bakta DB:
bakta_db: /path/to/bakta_db/db-light

# cgt executable parameters
cgt_exe: /path/to/bin/cgt_bacpop
...
Update where it says `/path/to/` variables to point to the correct locations. We will leave the rest of the parameters as they are for now.
...
bakta as the annotations are already present in `MAGs_for_CELEBRIMBOR/results`

```bash
snakemake --cores 4
```

You'll see a number of tools being run, including [CheckM](https://github.com/Ecogenomics/CheckM) for completeness calculation, [Panaroo](https://github.com/gtonkinhill/panaroo) for clustering, and [cgt](https://github.com/bacpop/cgt) for frequency threshold adjustment.
...
Using RStudio, you can plot the different between the adjusted and unadjusted results by updating the script `plot_frequency.R` with the paths to 
...
There are other files that can be used for downstream analysis, such as `presence_absence_matrix.txt` which defines in which genomes gene families are found, 
as well as the `annotated` directory, which contains gene annotations generated using Bakta.

### Further analysis

Try running CELEBRIMBOR with different parameters, such as `cgt_breaks` which defines the original rare and core thresholds to adjust, or `cgt_error`, which 
defines the false negative rate of CELEBRIMBOR. 

Also try setting `panaroo_stringency` to `moderate` or `senstive`. Details on the effect of these parameters can be found [here](https://gthlab.au/panaroo/#/gettingstarted/params). You can also try running with `clustering_method` set to `mmseqs2`, which is a more scalable but less accurate clustering method than Panaroo.
